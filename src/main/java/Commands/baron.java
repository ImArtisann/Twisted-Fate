...
public class baron extends Command {
    ...
    public baron(){
        this.name = "baron";
        this.help = "Use this command to bet on which team is going to get baron first which is a 4x multiplier  +baron " +
    ...
    protected void execute(CommandEvent event) {
        ...
        if(event.getArgs().isEmpty()) {
            event.reply("You didnt use the command correctly  +baron [Summoner's Name],[Bet amount],[Optional Region]");
        }else{
            ...
            try{
                ...
                System.out.println("A user cause an error in the baron command: " + e);
                event.reply("please make sure command is like so +baron [Summoner's Name],[Bet amount],[Optional Region]");
            }
        }
    ...
    public void placeBet(String userID, long bet, Summoner summoner, CommandEvent event){
        ...
        if(betTracking.baron.containsKey(userID)){
...
//                }
        ...
        if(betAlreadyPlaced == true){
            ...
            if(betTracking.baron.containsKey(userID)) {
                checker = betTracking.baron.get(userID);
            }
            Document doc = (Document) db.mongoCo.find(new Document("userID", userID)).first();
            long balance = doc.getLong("balance");
            ...
            if(betTracking.baron.containsKey(userID)) {
                betTracking.baron.remove(userID);
            }
            betTracking.baron.put(userID,checker);
        }
    ...
    public void betLogger(CommandEvent event, String userID, long bet, Summoner summoner){
        ...
        eb.addField("The user is hoping the summoner ", "Kills baron first", false);
    ...
    public void results(Summoner summoner, long gameID, String userID, long bet, CommandEvent event) throws RiotApiException{
        ...
        if(team<5){
            if(api.api.getMatch(summonerPlatform, gameID).getTeamByTeamId(100).isFirstBaron()){
                long win = updateDatabaseWin(userID, bet);
                event.reply("<@" + userID + "> The summoner " + summoner.getName() + " team did get the first baron for guessing correctly you have won " + win);
                postResults(userID,win,"win", event);
            }else {
                event.reply("<@" + userID + "> The summoner " + summoner.getName() + " team did not get the first baron");
                postResults(userID,bet,"lose", event);
            }
        }else{
            if(api.api.getMatch(summonerPlatform, gameID).getTeamByTeamId(200).isFirstBaron()){
                long win = updateDatabaseWin(userID, bet);
                event.reply("<@" + userID + "> The summoner " + summoner.getName() + " team did get the first baron for guessing correctly you have won " + win);
                postResults(userID,win,"win", event);
            }else {
    ...
    public void postResults(String userID,long bet, String winorLost, CommandEvent e) {
        ...
        if (winorLost.equalsIgnoreCase("win")) {
            ...
            eb.setDescription("The user has won his bet on baron");
            eb.addField("Total winnings: ", bet + " LP", true);
        } else {
            eb.setColor(Color.RED);
            eb.appendDescription("The user has lost his bet on baron");
    ...
    public long updateDatabaseWin(String userID, long bet){
        ...
        long winningbet = winning + winning;
